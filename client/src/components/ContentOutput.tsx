...
function PlatformSection({
    ...
    if (Array.isArray(val)) {
      return val.map((item) => {
        ...
        if (typeof item === "object")
          return item.post || item.text || item.content || JSON.stringify(item);
        return String(item);
      });
    }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between pr-4 hover:bg-slate-50">
    ...
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4">
      ...
      <div className="flex justify-between items-start mb-2">
        ...
        <Button
          ...
          size="sm"
          onClick={handleCopy}
          className={copied ? "text-green-600" : "text-slate-400"}
        >
          {copied ? (
            <>
      ...
      <Textarea
        ...
        className="min-h-[120px] bg-transparent border-0 focus-visible:ring-0 p-0 resize-y whitespace-pre-wrap"
      />
